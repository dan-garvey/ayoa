...
ORIGINAL = HERE.parent / "authored_checkups_20260918"
...
def prepare():
    ...
    comparison = {}
    for name in ("correction", "development", "combined", "reframe"):
        old = (ORIGINAL / "notes" / f"{name}.txt").read_text()
        new = (notes / f"{name}.txt").read_text()
        assert len(new.split()) <= len(old.split())
        comparison[name] = {
            "original_words": len(old.split()),
            "variant_words": len(new.split()),
            "original_sha256": hashlib.sha256(old.encode()).hexdigest(),
            "variant_sha256": hashlib.sha256(new.encode()).hexdigest(),
        }
    write(HERE / "note_comparison.json", comparison)
    ...
    assert checkpoint == read(ORIGINAL / "checkpoint.json")
    assert read(HERE / "inputs.json") == read(ORIGINAL / "inputs.json")
    assert hashes(HERE / "sources") == hashes(ORIGINAL / "sources")
    conditions = {}
    ...
    packets = [
        {"experiment": experiment, "sample": f"sample-{index:02d}"}
        for experiment in (ORIGINAL.name, HERE.name)
        for index in range(1, 11)
    ]
    random.SystemRandom().shuffle(packets)
    write(
        HERE / "comparison_mapping.json",
        {f"packet-{index:02d}": packet for index, packet in enumerate(packets, 1)},
    )
    ...
    source_files = [
        ...
        "inputs.json",
        "conditions.json",
        "comparison_mapping.json",
        "note_comparison.json",
        "resume.py",
        "audit.py",
        "verify_native.py",
