"""Smoke tests for the hand-authored Unblessed Summon checkpoint.
...
def test_checkpoint_loads_at_current_schema(checkpoint: CheckpointFile) -> None:
    ...
    assert checkpoint.importer_version == "synth-2026.05-unblessed"
...
def test_runtime_defaults_match_current_policy(checkpoint: CheckpointFile) -> None:
    ...
    assert checkpoint.config.models.model_dump() == expected_models
    assert checkpoint.session.config.settings.ticks_enabled is False
    assert checkpoint.config.settings.ticks_enabled is False
...
def test_character_descriptions_are_split_and_seeded(by_id: dict) -> None:
    for cid, char in by_id.items():
        assert char.agent_tier == CharacterAgentTier.plot, cid
