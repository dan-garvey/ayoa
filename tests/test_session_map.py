...
@pytest.fixture
def smap(tmp_path: Path) -> SessionMap:
...
class TestSessionRowCrud:
    ...
    def test_get_missing_returns_none(self, smap: SessionMap):
        assert arun(smap.get(channel_id=12345)) is None

    def test_upsert_then_get(self, smap: SessionMap):
        arun(smap.upsert(
            channel_id=12345,
            guild_id=999,
            session_id="sess-a",
        ...
        row = arun(smap.get(channel_id=12345))
    ...
    def test_upsert_overwrites(self, smap: SessionMap):
    ...
    def test_delete_returns_true_when_present(self, smap: SessionMap):
        ...
        assert arun(smap.delete(12345)) is True
        assert arun(smap.get(12345)) is None

    def test_delete_returns_false_when_absent(self, smap: SessionMap):
        assert arun(smap.delete(99999)) is False

    def test_delete_also_drops_pov_threads(self, smap: SessionMap):
        ...
        arun(smap.delete(12345))

        assert arun(smap.get(12345)) is None
        # Both pov_thread rows for channel 12345 are gone…
        assert arun(smap.get_pov_thread_id(12345, 42)) is None
        assert arun(smap.get_pov_thread_id(12345, 43)) is None
        # …but rows keyed to a DIFFERENT channel survive.
        assert arun(smap.get_pov_thread_id(99999, 42)) == 111
...
class TestPovThreads:
    def test_get_missing_returns_none(self, smap: SessionMap):
        ...
        assert arun(smap.get_pov_thread_id(channel_id=1, user_id=2)) is None

    def test_set_then_get(self, smap: SessionMap):
        arun(smap.set_pov_thread(
            channel_id=10, user_id=20, thread_id=999, character_id="hero",
        ))
        assert arun(smap.get_pov_thread_id(channel_id=10, user_id=20)) == 999

    def test_set_overwrites_existing(self, smap: SessionMap):
        ...
        assert arun(smap.get_pov_thread_id(channel_id=10, user_id=20)) == 222

    def test_set_keyed_by_channel_and_user(self, smap: SessionMap):
        ...
        assert arun(smap.get_pov_thread_id(channel_id=10, user_id=20)) == 111
        assert arun(smap.get_pov_thread_id(channel_id=10, user_id=21)) == 222
        assert arun(smap.get_pov_thread_id(channel_id=11, user_id=20)) == 333

    def test_clear_drops_row(self, smap: SessionMap):
        arun(smap.set_pov_thread(
            channel_id=10, user_id=20, thread_id=999, character_id="hero",
        ))
        arun(smap.clear_pov_thread(channel_id=10, user_id=20))
        assert arun(smap.get_pov_thread_id(channel_id=10, user_id=20)) is None

    def test_clear_missing_is_noop(self, smap: SessionMap):
        """Clearing a row that was never set must not raise — the
        bot calls clear unconditionally on certain failure paths."""
        arun(smap.clear_pov_thread(channel_id=999, user_id=999))

    def test_clear_all_drops_only_that_channel(self, smap: SessionMap):
        ...
        dropped = arun(smap.clear_all_pov_threads(channel_id=10))

        assert dropped == 2
        assert arun(smap.get_pov_thread_id(10, 20)) is None
        assert arun(smap.get_pov_thread_id(10, 21)) is None
        # Other channel's POV thread survives.
        assert arun(smap.get_pov_thread_id(11, 20)) == 333
        # Session binding survives — /clear is not /session end.
        assert arun(smap.get(10)) is not None

    def test_clear_all_on_empty_returns_zero(self, smap: SessionMap):
        ...
        assert arun(smap.clear_all_pov_threads(channel_id=99999)) == 0
    ...
    def test_init_is_idempotent(self, tmp_path: Path):
        ...
        arun(sm.init())
        arun(sm.init())
        arun(sm.set_pov_thread(
            channel_id=1, user_id=1, thread_id=42, character_id="x",
        ))
        assert arun(sm.get_pov_thread_id(channel_id=1, user_id=1)) == 42
...
class TestTurnMessages:
    def test_record_and_list_by_turns(self, smap: SessionMap):
        arun(smap.record_turn_message(
            channel_id=10,
            session_id="s1",
            turn_index=3,
        ...
        refs = arun(smap.list_turn_messages(
            channel_id=10,
            session_id="s1",
            turns=[3, 4],
        ))
    ...
    def test_forget_removes_only_named_refs(self, smap: SessionMap):
        ...
        assert arun(smap.forget_turn_messages(refs)) == 1

        remaining = arun(smap.list_turn_messages(
            channel_id=10,
            session_id="s1",
            turns=[3, 4],
        ))
    ...
    def test_session_delete_drops_turn_messages(self, smap: SessionMap):
        arun(smap.upsert(
        ...
        assert arun(smap.list_turn_messages(
            channel_id=10, session_id="s1", turns=[3],
        )) == []
        other = arun(smap.list_turn_messages(
            channel_id=11, session_id="s2", turns=[3],
        ))
        assert [r.message_id for r in other] == [1001]

    def test_clear_all_turn_messages_is_channel_scoped(
        ...
        assert arun(smap.clear_all_turn_messages(10)) == 1
        assert arun(smap.list_turn_messages(
            channel_id=10, session_id="s1", turns=[3],
        )) == []
        other = arun(smap.list_turn_messages(
