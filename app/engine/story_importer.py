...
Three sequential LLM calls that share a cached source-prompt prefix:
  1. World extraction (setting, lore, scenes, rules, hidden content)
  2. Character extraction (full roster, is_player flag)
...
validity server-side. Each call is issued in sequence so the shared source
prefix in the system prompt reads from cache on calls 2 and 3.
...
async def run_import(
    ...
    """Run the three-stage import pipeline and return the assembled checkpoint.
    ...
    """
    logger.info(
        "Source prompt: %d chars, ~%d words", len(source_text), len(source_text.split())
    )

    world = await extract_world(client, source_text)
    roster = await extract_characters(client, source_text)
    opening = await extract_opening(client, source_text)

    return build_checkpoint(world, roster, opening, story_id)
